...
function displayResults(keywordData) {
    ...
    if (Object.keys(keywordData).length > 0) {
      ...
      for (const cityName in keywordData) {
        if (keywordData.hasOwnProperty(cityName)) {
          // Get the page numbers and total count for the keyword
          const pageNumbers = keywordData[cityName].pageNumbers;
          const totalCount = keywordData[cityName].totalCount;
          ...
          keywordNameH3.textContent = cityName;
