...
// Function to create a PDF from selected pages
async function createPDF(pages, keyword) {
    const pdfDoc = await PDFDocument.create();
    const page = pdfDoc.addPage([612, 792]);

    const { font, drawText } = await PDFDocument.createFont(StandardFonts.Helvetica);
    page.setFont(font);
    ...
    downloadPDF(pdfURL, `Keyword_${keyword}.pdf`);
}
...
function addPDFButtons(keywordData) {
    for (const keyword in keywordData) {
        if (keywordData.hasOwnProperty(keyword)) {
            ...
            const createPDFButton = addCreatePDFButton(keyword, data.pageNumbers);
            // Append the button to the results
...
function searchPDF() {
    ...
    const ignoreKeywords = Array.from(
        document.querySelectorAll('input[name="ignoreKeyword"]:checked')
    );
    const keywordData = {};
                ...
                pdf.getPage(i).then((page) => {
                    page.getTextContent().then((content) => {
                        ...
                        keywords.forEach((keywordInput) => {
                            ...
                            // If the keyword is not in the ignore list and is found, store the page information
                            if (!ignoreKeywords.some(ignored => ignored.value === keyword) && text.includes(keyword)) {
...
function displayResults(keywordData) {
    ...
    // Add buttons for creating PDFs
    addPDFButtons(keywordData);
}
